...
bool enableVectorClocks = ENABLE_VECTOR_CLOCKS;
...
{  
  ...
  if(count > 10000000)
    count = 0; 
  //If pin is not high, skip loop body. 
  if(!digitalRead(FF_INT_PIN))
     return;  
  ...
  {   char incoming; 
      ...
      LOG_PORT.println(incoming); 
      //delay(300);
      if(incoming == 'a'){
        LOG_PORT.println("Restarting!!!");
  ...
  //Added start and stop characters to the message for simplicity
  if ( enableSerialLogging || enableSDLogging)
  {   LOG_PORT.write('*'); 
      Serial2.write('*'); 
      //Serial2.print(count, DEC);
      //Serial2.print(","); 
      //LOG_PORT.print(count, DEC);  
      //LOG_PORT.print(","); 
...
{uint8_t buffer[64]; 
  ...
  index = 0; 
  //tmp = sizeof(int); 
  //LOG_PORT.println(tmp); 
  if (enableAccel) // If accelerometer logging is enabled
  { 
    if ( enableCalculatedValues ) // If in calculated mode
    {  tmp = 1000000*imu.calcAccel(imu.ax);
       //LOG_PORT.println(imu.calcAccel(imu.ax), DEC); 
       LOG_PORT.println(tmp, DEC); 
       memcpy(buffer+index, &tmp, len); index+=4; 
       tmp = 1000000*imu.calcAccel(imu.ay);
       memcpy(buffer+index, &tmp , len); index+=4;  
       tmp = 1000000*imu.calcAccel(imu.az);
       memcpy(buffer+index, &tmp , len); index+=4; 
    }
    else
    {  tmp = 1000000*imu.ax; 
  ...
  LOG_PORT.println("Got Accel"); 
  for(i = 0; i < (index); i++){
    LOG_PORT.print(buffer[i], HEX); 
  }
  LOG_PORT.println(); 
  for(i = 0; i < 3; i++){
    LOG_PORT.print(buffer[i*4], HEX); 
    LOG_PORT.print(' '); 
  }
  LOG_PORT.println("End Accel"); 
  
  /*if (enableGyro) // If gyroscope logging is enabled
  {
    if ( enableCalculatedValues ) // If in calculated mode
    {
       memcpy(buffer+index, imu.calcGyro(imu.gx), len); index+=8; 
       memcpy(buffer+index, imu.calcGyro(imu.gy), len);index+=8; 
       memcpy(buffer+index, imu.calcGyro(imu.gz), len);index+=8; 
    }
    else
    {
       memcpy(buffer+index, (imu.gx), len);index+=8; 
       memcpy(buffer+index, (imu.gy), len);index+=8; 
       memcpy(buffer+index, (imu.gz), len);index+=8; 
    }
  }
  ...
  if (enableQuat) // If quaternion logging is enabled
  {
    ...
    {
      memcpy(buffer+index, imu.calcQuat(imu.qw), len); index+=8; 
      memcpy(buffer+index, imu.calcQuat(imu.qx), len); index+=8; 
      memcpy(buffer+index, imu.calcQuat(imu.qy), len); index+=8; 
      memcpy(buffer+index, imu.calcQuat(imu.qz), len); index+=8; 
    }
    else
    {   memcpy(buffer+index, (imu.qw), len);index+=8; 
       memcpy(buffer+index, (imu.qx), len);index+=8; 
       memcpy(buffer+index, (imu.qy), len);index+=8; 
       memcpy(buffer+index, (imu.qz), len);index+=8;   
    }
  }*/
...
void logIMUData(void)
{
 ...
 if(enableVectorClocks) // If vector clock logging enabled
  { 
    imuLog += String(count) + ", "; //Ugly hack at the moment... need to add this to IMU object
 }
  ...
  {  
    ...
    // LOG_PORT.print(", "); 
    // LOG_PORT.print(imuLog.length());
     
     Serial2.print(imuLog);  //Print to auxiliary port as well
     //Serial2.print(", "); 
     //Serial2.print(imuLog.length());
...
void initHardware(void)
{
  ...
  //Set up Firefly pin input 
  pinMode(FF_INT_PIN, INPUT); 
  //attachInterrupt(FF_INT_PIN, FF_handler, CHANGE); 
...
void FF_handler(){
  LOG_PORT.println("Running Handler!!"); 
  int_flag = 1; 
  //logIMUData(); 
}
